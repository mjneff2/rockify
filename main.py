...
def recommend_artist():
    ...
    result = None
    try:
        stmt = sqlalchemy.text("SELECT * FROM ArtistLikes WHERE Username = :usernameToCheck")
        with db.connect() as conn:
...
def recommend_album():
    ...
    for albumId in result['AlbumId']:
        ...
        try:
            ...
            with db.connect() as conn:
                trackProps = conn.execute(stmt, albumIdToCheck=albumId).first()
        except Exception as e:
            print(e)
        
        # Update avgTrackProperties with data of current track
        for i in range(len(avgTrackProperties)):
            avgTrackProperties[i][0] = (avgTrackProperties[i][0] * avgTrackProperties[i][1] + trackProps[avgTrackProperties[2]]) / (1 + avgTrackProperties[i][1])
