...
class Database:
    ...
    def get_tracks_by_popularity_and_artist_popularity(self, artistPopularityRating, trackPopularityRating):
        if artistPopularityRating is None or trackPopularityRating is None:
            return
        try:
            stmt = sqlalchemy.text("SELECT DISTINCT t.TrackName, t.Popularity FROM Track t WHERE EXISTS (SELECT * FROM Artist a JOIN TrackArtist ta ON a.ArtistId = ta.ArtistId WHERE t.TrackId = ta.TrackId AND t.Popularity > :trackPopularityToAdd AND a.Popularity > :artistPopularityToAdd) ORDER BY t.Popularity DESC LIMIT 10")
            with self.db.connect() as conn:
    ...
    def get_albums_by_average_tempo(self, tempo):
        if tempo is None:
            return
        try:
            stmt = sqlalchemy.text("SELECT DISTINCT al.AlbumName, ROUND(AVG(tp.Tempo)) AS avgTrackPopularity FROM Album al JOIN Track t ON al.AlbumId = t.AlbumId JOIN TrackProperties tp ON t.TrackId = tp.TrackId GROUP BY al.AlbumName HAVING avgTrackPopularity > :tempoToAdd ORDER BY AlbumName DESC LIMIT 10")
            with self.db.connect() as conn:
