...
class Database:
    ...
    def get_tracks_by_popularity_and_artist_popularity(self, artistPopularityRating, trackPopularityRating):
        if artistPopularityRating is None or trackPopularityRating is None:
            return
        try:
            stmt = sqlalchemy.text("SELECT t.TrackName FROM Tracks t WHERE EXISTS (SELECT * FROM Artist a JOIN TrackArtist ta ON a.ArtistId = ta.ArtistId WHERE t.TrackId = ta.TrackId AND t.Popularity > :trackPopularityToAdd AND a.Popularity > :artistPopularityToAdd) ORDER BY t.TrackName DESC LIMIT 10")
            with self.db.connect() as conn:
